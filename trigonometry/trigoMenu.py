...
from src import setCanvas2, CustomWidget, CustomLabel, Toast, changeWindow, changeSound
...
class TrigoMenu(Widget):
    ...
    def allButton(self):
        ...
        if self.name == "Field":
            ...
            self.rotateBtn.func_binder = lambda : changeWindow(self)
            self.changeTheme.func_binder = lambda : self.changeThemeFunc()
            self.soundBtn.func_binder = lambda : changeSound()
